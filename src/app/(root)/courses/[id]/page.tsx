...
export default async function CoursePage({
  ...
  return (
    <>
      <section className="relative w-full min-h-[70vh] md:min-h-[80vh] lg:min-h-screen flex items-start justify-center pt-24 md:pt-28 pb-14 md:pb-16 px-4 sm:px-6 md:px-10 bg-[url('/course.png')] bg-top bg-cover md:bg-contain bg-no-repeat">
        <div className="max-w-7xl w-full flex flex-col gap-12 md:gap-14 relative">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 lg:gap-16 items-start">
            <div className="flex flex-col justify-start text-left max-w-xl -mt-2 md:-mt-4 lg:-mt-6 md:pl-2 lg:pl-6">
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 items-start sm:items-center mb-3 md:mb-4">
                <Button
                  variant="secondary"
                  className="font-semibold px-8 sm:px-10 py-3 rounded-md shadow-md text-sm sm:text-base transition-colors w-full sm:w-auto"
                >
                  Join the Program
                </Button>
                <a
                  href="#"
                  className="underline text-white/90 text-xs sm:text-sm font-medium tracking-wide"
                >
                  OR DOWNLOAD SYLLABUS
                </a>
              </div>
            </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mt-2 md:mt-0">
            {[
              ...
              { label: "Program Duration", value: course.additionalInfo && typeof course.additionalInfo === 'object' && 'duration' in course.additionalInfo ? String(course.additionalInfo.duration) : "N/A", icon: "🕒" },
              { label: "Projects", value: "24+", icon: "🚀" },
            ].map((stat) => (
              <div
                key={stat.label}
                className="bg-white rounded-xl shadow p-4 sm:p-5 md:p-6 flex flex-col items-center justify-center text-center"
              >
