          ...
          <Card key={course.id} className="group overflow-hidden border-border/60 hover:shadow-sm transition-shadow">
            <CardHeader className="p-0">
              <div className="relative aspect-[16/9] overflow-hidden">
                ...
                <span className="absolute top-2 left-2 rounded-full bg-background/80 backdrop-blur px-2.5 py-1 text-[10px] sm:text-[11px] font-medium tracking-wide line-clamp-1">{course.syllabus.subtitle}</span>
              </div>
            </CardHeader>
            <CardContent className="pt-3 sm:pt-4 flex flex-col gap-2.5 sm:gap-3">
              <Stars value={course.additionalInfo.rating} />
              ...
              <div className="flex flex-wrap gap-1.5 sm:gap-2 text-[10px] sm:text-[11px]">
                {course.additionalInfo.duration && (
                  <span className="px-2 py-1 rounded-md bg-secondary/15 text-secondary font-medium">{course.additionalInfo.duration}</span>
                )}
                {course.additionalInfo.mode?.map(m => (
